...
#else
    const char* pszModule = "backspacecoin";
#endif
    if (pex)
        return strprintf(
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "backspacecoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "backspacecoind.pid"));
